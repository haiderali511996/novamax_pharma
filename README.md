...
- **Purchases**: Suppliers, manufacturers, purchase orders
...
- **Ledgers**: Running-balance account statements per Customer, Manufacturer, and Employee, plus a categorized Expense register. Customer invoices and paid payroll runs auto-post ledger entries; everything else (manufacturer bills/payments, employee advances) is entered manually
...
Resources: `products`, `batches`, `warehouses`, `stock-movements`, `customers`, `suppliers`, `manufacturers`, `sales-orders`, `purchase-orders`, `invoices`, `territories`, `distributors`, `distributor-invoices`, `employees`, `attendance`, `leaves`, `payroll`, `accounts`, `transactions`, `expenses`, `licenses`, `users`.
...
- `GET /api/ledger-entries/statement?partyType=customer|manufacturer|employee&party=<id>` — running-balance statement for one party
